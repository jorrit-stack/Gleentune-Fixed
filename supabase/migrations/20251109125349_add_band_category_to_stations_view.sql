/*
  # Add band_category to stations_view
  
  1. Changes
    - Drop existing stations_view
    - Recreate with band_category column
    - Set 'terrestrial' for all stations from stations, shortwave_stations tables (physical transmitters)
    - Use actual band_category from radio_stations table (can be 'terrestrial' or 'internet')
  
  2. Purpose
    - Enable frontend to filter by station category
    - Support hybrid product: terrestrial simulation + internet directory
...
-- Recreate view with band_category
CREATE VIEW stations_view AS
 SELECT s.station_id::text AS station_id,
    ...
    'terrestrial'::text AS band_category, -- FM/AM are always terrestrial
    s.created_at,
    s.updated_at
   FROM stations s
...
UNION ALL
 SELECT 'sw_'::text || sw.sw_station_id::text AS station_id,
    ...
    'terrestrial'::text AS band_category, -- SW is always terrestrial (physical transmitters)
    sw.created_at,
    sw.updated_at
...
UNION ALL
 SELECT 'legacy_'::text || rs.id::text AS station_id,
    ...
    COALESCE(rs.band_category, 'terrestrial'::text) AS band_category, -- Use actual band_category from table
    rs.created_at,
